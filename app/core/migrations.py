    ...
    async with engine.begin() as conn:
        try:
            ...
            if "duplicate column" in str(e).lower() or "already exists" in str(e).lower():
                ...
                print(f"⚠ Migration 45 error: {e}")
    
    # Migration 46: Add show_on_dashboard column to pools
    async with engine.begin() as conn:
        try:
            await conn.execute(text("""
                ALTER TABLE pools 
                ADD COLUMN show_on_dashboard BOOLEAN DEFAULT TRUE
            """))
            print("✓ Added show_on_dashboard column to pools")
        except Exception as e:
            # Column already exists
            if "duplicate column" in str(e).lower() or "already exists" in str(e).lower():
                pass  # Column exists, migration already ran
