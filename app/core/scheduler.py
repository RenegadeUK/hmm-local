...
class SchedulerService:
    ...
    def start(self):
        ...
        self.scheduler.add_job(
            self._execute_scheduled_cloud_backups,
            IntervalTrigger(minutes=1),
            id="execute_scheduled_cloud_backups",
            name="Execute scheduled cloud backups"
        )
    ...
    async def _execute_scheduled_cloud_backups(self):
        """Execute scheduled cloud backups"""
        try:
            from core.database import AsyncSessionLocal, CloudBackupConfig, CloudBackupLog
            ...
            async with AsyncSessionLocal() as db:
                ...
                result = await db.execute(
                    select(CloudBackupConfig).where(
                        CloudBackupConfig.enabled == True,
                        CloudBackupConfig.schedule_enabled == True
                    )
                )
                ...
                for config in configs:
                    ...
                    if config.schedule_frequency == "hourly":
                        # Backup every hour
                        if not config.last_backup or (now - config.last_backup).total_seconds() >= 3600:
                            should_backup = True
                    
                    elif config.schedule_frequency == "daily":
                        # Backup at specific time each day
                        if config.schedule_time:
                            target_hour, target_minute = map(int, config.schedule_time.split(':'))
                            if now.hour == target_hour and now.minute == target_minute:
                                # Check we haven't backed up in the last hour
                                if not config.last_backup or (now - config.last_backup).total_seconds() >= 3600:
                                    should_backup = True
                    
                    elif config.schedule_frequency == "weekly":
                        # Backup on specific day of week at specific time
                        if config.schedule_day is not None and config.schedule_time:
                            target_hour, target_minute = map(int, config.schedule_time.split(':'))
                        ...
                        # Backup on specific day of month at specific time
                        if config.schedule_day is not None and config.schedule_time:
                            target_hour, target_minute = map(int, config.schedule_time.split(':'))
                            if (now.day == config.schedule_day and 
                                now.hour == target_hour and 
                                now.minute == target_minute):
                                if not config.last_backup or (now - config.last_backup).total_seconds() >= 3600:
                                    should_backup = True
                    ...
                    if should_backup:
                        logger.info(f"📤 Executing scheduled backup to {config.provider}")
                        ...
                        try:
                            ...
                            with tempfile.NamedTemporaryFile(mode='w', suffix='.json', delete=False) as tmp:
                                ...
                                from core.database import (
                                    Miner, Pool, AutomationRule, NotificationConfig, 
                                    AlertConfig, TuningProfile, CustomDashboard, DashboardWidget
                                )
                            ...
                            cloud_backup_manager.configure_provider(config.provider, {
                                "enabled": True,
                                **config.config
                            })
                            
                            success = await cloud_backup_manager.upload_backup(tmp_path, config.provider)
                            ...
                            logger.info(f"✅ Scheduled backup to {config.provider} completed: {success}")
                            
                        except Exception as e:
                            logger.error(f"❌ Scheduled backup to {config.provider} failed: {e}")
            ...
            logger.error(f"❌ Failed to execute scheduled cloud backups: {e}")
            import traceback
            traceback.print_exc()
