"""\
BCH Stack (CKPool) Local Stratum Driver

Targets a "bch-stack" style single-container deployment:
- BCHN node (bitcoind + bitcoin-cli)
- CKPool stratum endpoint (default port 3334)
- Manager API (default port 8084) providing health/metrics at /api/v1/*
...
class BCHStackIntegration(BasePoolIntegration):
    """Integration for a local BCH stack CKPool stratum + manager API."""

    pool_type = "bch_stack"
    display_name = "BCH Stack (Local CKPool Stratum)"
    driver_version = __version__
    documentation_url = "https://github.com/RenegadeUK/hmm-local"
    supports_coins = ["BCH"]
    requires_api_key = False

    DEFAULT_STRATUM_PORT = 3334
    DEFAULT_MANAGER_PORT = 8084
    HTTP_TIMEOUT_S = 4.0
    TCP_TIMEOUT_S = 3.0
    ...
    def get_pool_templates(self) -> List[PoolTemplate]:
        return [
            PoolTemplate(
                template_id="bch_stack_local_bch",
                display_name="BCH Stack BCH (Local)",
                url="192.168.1.100",
                ...
                description="Local BCH stack CKPool stratum endpoint (edit URL to your host/IP)",
                fee_percent=0.0,
            )
        ]
