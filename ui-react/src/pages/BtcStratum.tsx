...
export default function BtcStratum() {
  const { data, isLoading, isError, error } = useQuery<StratumStatusResponse>({
    queryKey: ['stratum', 'BTC'],
    queryFn: () => stratumAPI.getStatus('BTC'),
  ...
  if (isLoading) {
    return <div className="w-full py-10 text-center text-sm text-muted-foreground">Loading BTC Stratum…</div>
  }

  if (isError || !data) {
    const message = error instanceof Error ? error.message : 'Unknown error'
    return <div className="rounded-lg border border-red-700 bg-red-900/30 p-4 text-red-200">Failed to load BTC Stratum: {message}</div>
  }
  ...
  return (
    <div className="space-y-4">
      <div>
        <h1 className="text-2xl font-semibold">BTC Stratum</h1>
