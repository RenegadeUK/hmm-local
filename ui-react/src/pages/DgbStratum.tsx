...
export default function DgbStratum() {
  const { data, isLoading, isError, error } = useQuery<StratumStatusResponse>({
    queryKey: ['stratum', 'DGB'],
    queryFn: () => stratumAPI.getStatus('DGB'),
  ...
  if (isLoading) {
    return <div className="w-full py-10 text-center text-sm text-muted-foreground">Loading DGB Stratum…</div>
  }

  if (isError || !data) {
    const message = error instanceof Error ? error.message : 'Unknown error'
    return <div className="rounded-lg border border-red-700 bg-red-900/30 p-4 text-red-200">Failed to load DGB Stratum: {message}</div>
  }
  ...
  return (
    <div className="space-y-4">
      <div>
        <h1 className="text-2xl font-semibold">DGB Stratum</h1>
