...
app = FastAPI(title="DGB Local Stack Manager", version="0.1.0")

logger = logging.getLogger("dgb_stack.manager")

CONFIG_ROOT = Path("/config")
NODE_CONF = CONFIG_ROOT / "node" / "digibyte.conf"
...
def run_digibyte_cli(method: str) -> dict[str, Any]:
  code, output, error = run_command(
    ["digibyte-cli", "-conf=/config/node/digibyte.conf", "-datadir=/config/node", method],
    retries=1,
  )
  if code != 0:
  ...
  try:
    ...
    return {"ok": False, "error": "invalid json from digibyte-cli", "raw": output}
...
def get_ckpool_internal_log_path() -> Path | None:
  ...
  # IMPORTANT: CKPool log filename is based on ckp.name, which defaults to "ckpool".
  # The JSON config "name" is NOT used for this unless ckpool is started with -n.
  default_internal_dir = CONFIG_ROOT / "logs" / "ckpool" / "logs"
  preferred = default_internal_dir / "ckpool.log"
  if preferred.exists():
    return preferred

  # If the user configured logdir explicitly, respect it.
  ...
  # Fallback: pick the newest *.log file in the default internal dir.
  try:
...
def node_sync_status() -> dict:
    result = run_digibyte_cli("getblockchaininfo")
    if not result["ok"]:
        return {
            "ok": False,
...
def health() -> dict:
    return {
        "status": "ok",
        "service": "dgb-local-stack-manager",
        "timestamp": datetime.utcnow().isoformat(),
    }
...
def get_services_v1() -> dict:
  return {
    ...
    "services": {
      ...
      "digibyted": structured_service_status("digibyted"),
...
def get_snapshot_v1(
  ...
  blockchain = run_digibyte_cli("getblockchaininfo")
  network = run_digibyte_cli("getnetworkinfo")
  mining = run_digibyte_cli("getmininginfo")
...
def restart_service(service_name: str) -> dict:
    allowed = {"digibyted", "ckpool", "manager"}
...
<head>
  ...
  <style>
    * { box-sizing: border-box; }
    ...
    .brand-dgb { color:#3b82f6; font-size:11px; vertical-align:super; margin-left:2px; font-weight:700; }
...
<body>
  ...
  <link rel="shortcut icon" href="/favicon.ico?v=4" />
    ...
    <div class="brand-text">
      <span class="brand-title">HMM Local<sup class="brand-dgb">DGB</sup></span>
      <span class="brand-subtitle">Home Miner Manager</span>
    </div>
  </div>
  <p>DGB Local Stack Manager • Coin fixed to DGB, algo fixed to sha256d.</p>
  ...
  <div class=\"card\">
    <div class="actions">
      ...
      <button onclick="restartSvc('digibyted')">Restart Node</button>
